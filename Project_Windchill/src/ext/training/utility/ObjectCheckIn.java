...
public class ObjectCheckIn implements RemoteAccess, Serializable {
	...
	public static void main(String[] args) throws Exception{
		...
		try {
				...
				new SearchCondition(WTPart.class, WTPart.NAME, SearchCondition.EQUAL,"Test Part"),new int[] {0});
			...
			while(allVersions.hasMoreElements())
			{
				if(islatest) //Validate if its the latest version or not
				{
					...
					if(WorkInProgressHelper.isCheckedOut(part)) 
					{
						...
						WorkInProgressHelper.service.checkin((Workable)part,"Checked In through code for training");
